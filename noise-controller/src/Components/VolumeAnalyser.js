...
class VolumeAnalyser extends React.Component {
	...
	componentDidMount() {
		axiosWithAuth().get(`https://noisecontroller.herokuapp.com/api/classes/`)
			.then(response => {
				this.setState({ classId: response.data[0].id })
				axiosWithAuth().get(`https://noisecontroller.herokuapp.com/api/classes/${this.state.classId}`)
					.then(response => {
						//console.log("YOU'RE SENSITIVE", response.data)
						if (response.data.sensitivity) this.setState({ maxVol: response.data.sensitivity })
	...
	handleChange = (e, value) => {
		axiosWithAuth().put(`https://noisecontroller.herokuapp.com/api/classes/${this.state.classId}`, { sensitivity: value })
			.then(response => {
