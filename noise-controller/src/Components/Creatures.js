...
function Creatures() {
	...
	useEffect(() => {
		...
		if (!isLoud && isPlaying) {
			...
			myTimer = setInterval(() => {
				setCurrentScore(score => (score + 5 > 200 ? 200 : score + 5));
				setImagesArr(images => [
					...images,
	...
	const finishGame = (e) => {
		...
		axiosWithAuth().post(`https://noisecontroller.herokuapp.com/api/classes/${classId}/scores`, { score: currentScore })
			.then(response => {
				console.log("SCOREPLEASE", response);
				setCurrentScore(0);
				setImagesArr([]);
			})
			.catch(error => { console.log(error.response) })
